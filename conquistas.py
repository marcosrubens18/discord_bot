# conquistas.py — Sistema de conquistas com recompensas
import discord
...
CONQUISTAS = [
    # ── Primeiros Passos ─────────────────────────────────────────
    ("primeira_batalha",  "Primeira Batalha",      "Venca sua primeira batalha",          "⚔️",  "vitorias",  1,    50,   30,  None),
    ("sobrevivente",      "Sobrevivente",           "Sobreviva a 10 batalhas",             "🛡️",  "vitorias",  10,   100,  60,  None),
    ("guerreiro_nato",    "Guerreiro Nato",         "Venca 50 batalhas",                   "🏆",  "vitorias",  50,   300,  200, "Incomum"),
    ("lendario_guerra",   "Lendario da Guerra",     "Venca 200 batalhas",                  "💎",  "vitorias",  200,  800,  500, "Raro"),
    ("transcendente",     "Transcendente",          "Venca 500 batalhas",                  "✨",  "vitorias",  500,  2000, 1500,"Epico"),
    ...
    ("rank_s",  "Lendario",       "Alcance o Rank S",   "⭐", "rank", 60, 2000, 1200,"Epico"),
    ("rank_ss", "Transcendente",  "Alcance o Rank SS",  "💎", "rank", 75, 5000, 3000,"Lendario"),

    # ── PvP ──────────────────────────────────────────────────────
    ("primeiro_pvp",      "Duelista",               "Venca seu primeiro duelo PvP",        "⚔️",  "pvp_vitorias", 1,  100,  70,  None),
    ("campeao_pvp",       "Campeao",                "Venca 10 duelos PvP",                 "🏆",  "pvp_vitorias", 10, 400,  300, "Raro"),
    ("gladiador",         "Gladiador",              "Venca 50 duelos PvP",                 "👑",  "pvp_vitorias", 50, 1500, 1000,"Epico"),

    # ── Economia ─────────────────────────────────────────────────
    ("rico",              "Comerciante",            "Acumule 1.000 moedas",                "💰",  "moedas",    1000,  80,  0,   None),
    ("milionario",        "Milionario",             "Acumule 10.000 moedas",               "💎",  "moedas",    10000, 300, 0,   "Incomum"),
    ("magnata",           "Magnata",                "Acumule 100.000 moedas",              "🤑",  "moedas",    100000,1000,0,   "Raro"),

    # ── Missoes ──────────────────────────────────────────────────
    ("dedicado",          "Dedicado",               "Complete 10 missoes diarias",         "📋",  "missoes_completas", 10,  150, 100, None),
    ("disciplinado",      "Disciplinado",           "Complete 30 missoes diarias",         "📅",  "missoes_completas", 30,  400, 300, "Incomum"),
]
...
async def get_conquistas_usuario(user_id):
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM conquistas WHERE user_id=$1", user_id)
        return {r["conquista_id"]: r for r in rows}

async def verificar_conquistas(user_id, tipo, valor_atual):
    """Verifica e completa conquistas do tipo especificado. Retorna lista de conquistas completas."""
    pool = await get_pool()
    concluidas = []
    async with pool.acquire() as conn:
        for cq in CONQUISTAS:
            ...
            row = await conn.fetchrow("SELECT * FROM conquistas WHERE user_id=$1 AND conquista_id=$2", user_id, cid)
            ...
            if valor_atual >= meta:
                ...
                concluidas.append({"nome": nome, "desc": desc, "emoji": emoji, "xp": xp, "moedas": moedas, "ficha": ficha})
            else:
                # Atualiza progresso
                await conn.execute("""
                    INSERT INTO conquistas (user_id, conquista_id, progresso, concluida, data)
...
async def cmd_conquistas(interaction: discord.Interaction):
    ...
    categorias = {
        ...
        "Missoes":   ["dedicado","disciplinado"],
    }

    EMOJI_FICHA = {"Incomum":"🟩","Raro":"🟦","Epico":"🟪","Lendario":"🟧"}
