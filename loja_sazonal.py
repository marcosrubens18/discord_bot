# loja_sazonal.py — Loja de itens sazonais + rotativa diaria
import discord
...
class LojaItemModal(discord.ui.Modal, title="Adicionar Item Sazonal"):
    ...
    preco_estoque = discord.ui.TextInput(
        label="Preco | Estoque (-1 = ilimitado)",
        placeholder="Ex: 2000|10  ou  500|-1",
        max_length=20
    )
    descricao = discord.ui.TextInput(
        label="Descricao do item",
        placeholder="Ex: Arma lendaria do inverno eterno",
        max_length=200
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("Formato invalido!", ephemeral=True); return
        try:
...
class LojaRotativaModal(discord.ui.Modal, title="Adicionar Item Rotativo"):
    item_info = discord.ui.TextInput(
        label="ID | Nome | Emoji | Tipo | Raridade",
        placeholder="Ex: pocao_exp|Pocao de XP|✨|pocao|Raro",
        max_length=150
    )
    preco_estoque = discord.ui.TextInput(
        label="Preco | Estoque",
    ...
    descricao = discord.ui.TextInput(
        label="Descricao",
        placeholder="Disponivel somente hoje!",
        max_length=150,
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            partes = [x.strip() for x in str(self.item_info).split("|")]
            item_id  = partes[0]; nome = partes[1] if len(partes)>1 else "Item"
            emoji    = partes[2] if len(partes)>2 else "📦"
            tipo     = partes[3] if len(partes)>3 else "material"
            raridade = partes[4] if len(partes)>4 else "Raro"
        except:
...
async def cmd_loja_sazonal(interaction: discord.Interaction):
    ...
    if not itens_saz and not itens_rot:
        await interaction.followup.send("Nenhum item disponivel no momento!", ephemeral=True); return
    ...
    async with pool.acquire() as conn:
        ...
        if not it:
            await interaction.followup.send("Item indisponivel!", ephemeral=True); return
        if not p2 or p2["moedas"] < it["preco"]:
            await interaction.followup.send(f"Moedas insuficientes! Precisa de {it['preco']} 🪙", ephemeral=True); return
...
async def cmd_loja_sazonal_remover(interaction: discord.Interaction, item_id_num: int):
    await interaction.response.defer(ephemeral=True)
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute("UPDATE loja_sazonal SET ativo=FALSE WHERE id=$1", item_id_num)
    await interaction.followup.send(f"Item #{item_id_num} removido da loja sazonal!", ephemeral=True)
