# sistema_sorteio.py — Sistema de sorteios comunitários
...
class CriarSorteioModal(discord.ui.Modal, title="🎲 Criar Sorteio"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Define texto da recompensa baseado no ID
        if self.item_id.startswith("moedas_"):
            qtd = self.item_id.split("_")[1]
            texto_recompensa = f"🪙 **{qtd} Moedas**"
        elif self.item_id.startswith("xp_"):
            qtd = self.item_id.split("_")[1]
...
async def finalizar_e_anunciar_sorteio(sorteio_id: int, guild):
    ...
    if sorteio["item_id"].startswith("moedas_"):
        ...
        texto_recompensa = f"🪙 **{qtd} Moedas**"
    elif sorteio["item_id"].startswith("xp_"):
        qtd = sorteio["item_id"].split("_")[1]
        texto_recompensa = f"⭐ **{qtd} XP**"
...
def get_todos_itens_para_sorteio():
    ...
    for m in [100, 500, 1000, 5000, 10000]:
        itens.append({
            "id": f"moedas_{m}", "nome": f"{m} Moedas",
            "raridade": "Comum" if m <= 500 else "Incomum" if m <= 1000 else "Raro" if m <= 5000 else "Epico",
            "emoji": "🪙", "categoria": "💰 Moedas"
        })
    
    # XP
    for x in [100, 500, 1000, 5000, 10000]:
        itens.append({
            "id": f"xp_{x}", "nome": f"{x} XP",
            "raridade": "Comum" if x <= 500 else "Incomum" if x <= 1000 else "Raro" if x <= 5000 else "Epico",
            "emoji": "⭐", "categoria": "⭐ Experiência"
        })
    
    # Fichas
    for rar in ["Comum", "Incomum", "Raro", "Epico", "Lendario"]:
        itens.append({
            "id": f"ficha_{rar.lower()}", "nome": f"Ficha {rar}",
            "raridade": rar, "emoji": "🎰", "categoria": "🎰 Fichas de Roleta"
        })
    
    # Poções
    from data_itens import POCOES_CAT
    for pocao in POCOES_CAT:
        itens.append({
            "id": pocao["id"], "nome": pocao["nome"],
            "raridade": pocao["raridade"], "emoji": pocao.get("emoji", "🧪"),
            "categoria": "🧪 Poções"
        })
    
    # Armas
    from data_armas import ARMAS_POR_CLASSE
    for classe, armas in ARMAS_POR_CLASSE.items():
        for arma in armas:
            itens.append({
                "id": arma["id"], "nome": arma["nome"],
                "raridade": arma["raridade"], "emoji": arma.get("emoji", "⚔️"),
                "categoria": f"⚔️ Armas - {classe.title()}"
            })
    
    # Armaduras
    from data_armaduras import ARMADURAS_POR_CLASSE
    for classe, armaduras in ARMADURAS_POR_CLASSE.items():
        for armadura in armaduras:
            itens.append({
                "id": armadura["id"], "nome": armadura["nome"],
                "raridade": armadura["raridade"], "emoji": armadura.get("emoji", "🛡️"),
                "categoria": f"🛡️ Armaduras - {classe.title()}"
            })
    
    # Materiais
    from data_itens import MATERIAIS_CAT
    for material in MATERIAIS_CAT:
        itens.append({
            "id": material["id"], "nome": material["nome"],
            "raridade": material["raridade"], "emoji": material.get("emoji", "📦"),
            "categoria": "📦 Materiais"
        })
...
async def autocomplete_item_sorteio(interaction: discord.Interaction, current: str):
    ...
    return [
        app_commands.Choice(
            name=f"{i['emoji']} {i['nome']} [{i['raridade']}] - {i.get('categoria', 'Item')}"[:100],
...
async def cmd_sorteios_listar(interaction: discord.Interaction):
    ...
    for s in sorteios:
        ...
        if s["item_id"].startswith("moedas_"):
            qtd = s["item_id"].split("_")[1]
            texto_recompensa = f"🪙 {qtd} Moedas"
        elif s["item_id"].startswith("xp_"):
            qtd = s["item_id"].split("_")[1]
            texto_recompensa = f"⭐ {qtd} XP"
