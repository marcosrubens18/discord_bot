# sistema_combate.py — Sistema de batalha completo (PVE + PVP)
...
from data_armas import get_bonus_arma
from data_armaduras import get_bonus_armadura
...
async def rodar_treino(interaction: discord.Interaction, p, monstro, arena):
    ...
    bonus_party = 0
    try:
        from sistema_party import get_bonus_party, registrar_batalha_party
    ...
    if vitoria:
        try:
            from sistema_eventos import registrar_batalha_evento
            await registrar_batalha_evento(uid)
        except:
            pass

        try:
            from sistema_passe import adicionar_pontos_batalha
    ...
    if vitoria:
        ...
        if loot:
            from sistema_personagem import add_loot
        ...
        try:
            from sistema_missoes import atualizar_progresso
        ...
        try:
            from sistema_conquistas import verificar_conquistas
            pool2 = await get_pool()
            async with pool2.acquire() as conn:
                p2 = await conn.fetchrow("SELECT vitorias, moedas FROM personagens WHERE user_id=$1", uid)
            cqs_vit = await verificar_conquistas(uid, "vitorias", p2["vitorias"] if p2 else 0)
            cqs_mon = await verificar_conquistas(uid, "moedas", p2["moedas"] if p2 else 0)
