...
from batalha import (
    ...
    EMOJI_CLASSE, salvar_resultado, MONSTROS
)
...
def get_monstro_por_nome(nome: str) -> dict:
    ...
    for m in MONSTROS:
        if m["nome"].lower() == nome_lower or nome_lower in m["nome"].lower():
            return m
    # Fallback: usa goblin se não encontrar
    return MONSTROS[0]
...
async def rodar_combate_expedicao(canal: discord.TextChannel, participantes_ids: list,
    ...
    Retorna dict com resultado: sobreviventes, derrotados, sucesso.
    """
    pool = await get_pool()
    ...
    embed_ini = discord.Embed(
        ...
        description=(
            f"Os aventureiros se preparam para enfrentar **{quantidade}x {monstro['nome']}**!\n\n"
            f"{barra_status_grupo(jogadores, hp_m, hp_mmx, monstro)}"
        ),
        color=0xE24B4A
    )
        ...
        for j in vivos:
            ...
            if acao == "timeout":
                ...
                await canal.send(embed=discord.Embed(
                    description=f"⏰ {j['nome']} perdeu o turno! ({tc}/3)", color=0xE4AF3C))
                continue
            else:
                timeout_count[uid] = 0
    ...
    embed_res = discord.Embed(
        ...
        description=(
            f"O {monstro['nome']} foi derrotado!\n\n" if sucesso else
            f"O grupo foi derrotado pelo {monstro['nome']}...\n\n"
        ) + (
