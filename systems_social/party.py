# systems/social/party.py — Sistema de Party
...
async def init_db_party():
    """Inicializa as tabelas de party"""
    pool = await get_pool()
...
async def get_party_do_jogador(user_id: int) -> Optional[dict]:
...
async def get_membros_party(party_id: int) -> List[dict]:
...
async def get_stats_party(party_id: int) -> dict:
...
async def get_bonus_party(participantes_ids: list, guild) -> int:
    """Retorna bônus de XP baseado na party"""
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with pool.acquire() as conn:
            party = await conn.fetchrow("""
                ...
                VALUES ($1, $2, $3)
                RETURNING *
            """, self.nome.value, interaction.user.id, MAX_PARTY_MEMBROS)
        ...
        embed_sucesso = discord.Embed(
            ...
            description=f"**{self.nome.value}** foi criada com sucesso!\n\n"
                       ...
                       f"👥 Limite: {MAX_PARTY_MEMBROS} membros\n\n"
...
async def cmd_party_convidar(interaction: discord.Interaction, jogador: discord.Member):
    ...
    class ConviteView(discord.ui.View):
        def __init__(self):
            super().__init__(timeout=TEMPO_CONVITE_PARTY)
    ...
    embed_convite = discord.Embed(
        ...
        description=f"**{interaction.user.display_name}** te convidou para entrar na party **{party['nome']}**!\n\n"
                   ...
                   f"⏱️ O convite expira em {TEMPO_CONVITE_PARTY // 60} minutos.",
        color=COR_PRIMARY
    )
