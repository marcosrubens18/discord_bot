...
class CriarEventoModal(discord.ui.Modal, title="Criar Novo Evento"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Duracao",  value=f"⏱️ {duracao_txt}", inline=True)
        embed.add_field(name="Termina",  value=f"<t:{int(fim.timestamp())}:R>", inline=True)
        embed.add_field(
            name=f"{premio_info['emoji']} Premio",
            value=str(self.premio_desc) or f"{premio_info['nome']}: {self.premio_valor}",
            inline=False
        )
        embed.set_footer(text=f"Evento #{evento['id']} | Use /evento-info para detalhes | /participar para entrar")
...
async def encerrar_evento(evento_id, guild):
    ...
    embed.add_field(name=f"{premio_info['emoji']} Premio entregue", value=f"{member_v.mention if member_v else vencedor['nome']}: {premio_entregue}", inline=False)
    embed.set_footer(text=f"Obrigado a todos que participaram!")
    if canal:
