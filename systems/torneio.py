# systems/torneio.py — Sistema de Torneios PvP
...
from data.constantes import ARENAS, COR_PRIMARY, COR_SUCCESS, COR_DANGER, COR_WARNING, COR_INFO, COR_GOLD
...
async def autocomplete_torneio_ativo(interaction: discord.Interaction, current: str):
    if not interaction.guild:
        return []
    
    pool = await get_pool()
    async with pool.acquire() as conn:
        torneios = await conn.fetch("""
            SELECT id, nome FROM torneios 
            WHERE guild_id = $1 AND status = 'inscricoes'
            ORDER BY id DESC
        """, interaction.guild.id)
    
    return [
        app_commands.Choice(
            name=f"#{t['id']} - {t['nome'][:50]}",
            value=str(t["id"])
        )
        for t in torneios if not current or current.lower() in t["nome"].lower()
    ][:25]