# systems/roleta.py — Sistema de Roletas e Gacha
...
from database.queries import get_personagem, get_skills_desbloqueadas, get_inventario
from data.constantes import COR_RAR, EMOJI_FICHA, RARIDADES
from data.racas import RACAS_ROLETA
from data.classes import CLASSES, PODERES
from data.skills import get_skills_classe
...
ROLETAS = {
    ...
    "skill": {
        "nome": "Skill", "emoji": "⚡",
        "pool": [],  # Será preenchido dinamicamente
    },
    "arma": {
        "nome": "Arma", "emoji": "⚔️",
        "pool": [],  # Será preenchido dinamicamente
    ...
    "poder": {
        ...
        "pool": [
            {"id": p["id"], "nome": p["nome"], "emoji": p["emoji"], "valor": p["valor"], "raridade": "Comum" if p["valor"] <= 18 else "Incomum" if p["valor"] <= 26 else "Raro" if p["valor"] <= 35 else "Epico" if p["valor"] <= 48 else "Lendario", "desc": p["desc"] if "desc" in p else ""}
            for p in PODERES
        ],
    },
}
