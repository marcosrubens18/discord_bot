# systems/dupla.py — Sistema de batalha em dupla
...
from database.queries import (
    get_personagem, get_skills_equipadas, get_pocoes_inv,
...
async def _processar_acao(acao, val, p, monstro, hp_m, hp_mmx, mana, mana_mx, hp_j, hp_jmx, ef_j, ef_m, skills, batk, passiva, uid):
    """Processa ação do jogador e retorna linha descritiva"""
    if acao == "atk_basico":
        ...
        return (hp_m, mana, hp_j, f"⚔️ Ataque Básico: **{dano} de dano**!")
...
async def rodar_treino_dupla(interaction: discord.Interaction, p1, p2, monstro, arena,
    ...
    # Setup P1
    ids1 = await get_skills_equipadas(uid1)
    skills1 = []
    for sid in ids1:
    ...
    # Setup P2
    ids2 = await get_skills_equipadas(uid2)
    skills2 = []
    for sid in ids2:
    ...
    if vitoria:
        # Bônus de 20% por batalha em dupla
        ...
        embed_fim = discord.Embed(
            ...
            description=(
                ...
                f"+{xp_bonus} XP | +{moedas_bonus} 🪙 (+20% bônus dupla)"
            ),
            color=0x1D9E75
        )
    else:
