# systems/hospital.py — Sistema de Hospital e Cura
...
from data.constantes import PLANOS_HOSPITAL, IMG_HOSPITAL
...
async def get_descanso(user_id: int):
    ...
    async with pool.acquire() as conn:
        ...
        if not row:
            return None
        if datetime.now() >= row["proximo_descanso"]:
            return None
...
async def usar_descanso(user_id: int):
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        proximo = datetime.now() + timedelta(minutes=30)
        await conn.execute("""
...
async def cmd_hospital(interaction: discord.Interaction):
    ...
    if not pode_descansar and proximo_desc:
        secs = max(0, int((proximo_desc - datetime.now()).total_seconds()))
        mins = secs // 60
        segs = secs % 60
