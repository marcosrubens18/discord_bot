# systems/social/guildas.py — Sistema de Guildas
...
from data.constantes import COR_PRIMARY, COR_SUCCESS, COR_DANGER, COR_WARNING, COR_INFO, COR_GOLD
...
async def init_db_guildas():
    """Inicializa as tabelas de guildas"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        ...
        # Migrações
        migracoes = [
            ("guildas",       "nivel",    "INTEGER", "DEFAULT 1"),
            ("guildas",       "xp",       "INTEGER", "DEFAULT 0"),
            ("guildas",       "canal_id", "BIGINT",  "DEFAULT 0"),
            ("guilda_missoes","semana",   "DATE",    "DEFAULT CURRENT_DATE"),
            ("guilda_missoes","tipo",     "TEXT",    "DEFAULT ''"),
            ("guilda_missoes","premio_xp","INTEGER", "DEFAULT 0"),
            ("guilda_missoes","premio_moedas","INTEGER","DEFAULT 0"),
        ]
        for tabela, col, tipo, default in migracoes:
            try:
                await conn.execute(f"ALTER TABLE {tabela} ADD COLUMN IF NOT EXISTS {col} {tipo} {default}")
            except Exception:
                pass
...
async def get_guilda_do_jogador(user_id: int) -> tuple:
    """Retorna a guilda do jogador e seu membro"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        ...
        return dict(g), dict(m)


async def get_membros(guilda_id: int) -> List[dict]:
    """Retorna lista de membros da guilda"""
    pool = await get_pool()
    async with pool.acquire() as conn:
...
def get_bonus_guilda(nivel: int) -> dict:
    """Retorna os bônus da guilda baseado no nível"""
...
async def atualizar_missao_guilda(user_id: int, tipo: str, quantidade: int = 1) -> Optional[tuple]:
    """Atualiza progresso de missão semanal da guilda"""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            ...
            for miss in missoes:
                ...
                if novo_prog >= miss["meta"]:
                    ...
                    if guilda and guilda["canal_id"]:
                        return (guilda["canal_id"], miss["descricao"], miss["premio_xp"], miss["premio_moedas"])
                else:
...
async def gerar_missoes_semanais(guilda_id: int) -> bool:
...
async def criar_canal_guilda(guild: discord.Guild, nome: str, emoji: str, cargo: discord.Role, bot_member) -> Optional[discord.TextChannel]:
...
class CriarGuildaModal(discord.ui.Modal, title="Criar Guilda"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with pool.acquire() as conn:
            ...
            if ex_m:
                await interaction.followup.send("Voce ja faz parte de uma guilda! Saia primeiro.", ephemeral=True)
                return
            ex_n = await conn.fetchrow("SELECT id FROM guildas WHERE LOWER(nome) = $1", nome.lower())
            if ex_n:
                await interaction.followup.send(f"Ja existe uma guilda chamada **{nome}**!", ephemeral=True)
                return
            ...
            if p["moedas"] < CUSTO_CRIAR:
                await interaction.followup.send(f"Precisa de **{CUSTO_CRIAR} moedas**! Voce tem {p['moedas']}.", ephemeral=True)
                return
        ...
        # Gerar missoes semanais iniciais
...
async def cmd_guilda_info(interaction: discord.Interaction, nome: str = ""):
    ...
    async with pool.acquire() as conn:
        ...
        if not g:
            await interaction.followup.send("Guilda nao encontrada!", ephemeral=True)
            return
...
async def cmd_guilda_missoes(interaction: discord.Interaction):
    ...
    if not g:
        await interaction.followup.send("Voce nao faz parte de nenhuma guilda!", ephemeral=True)
        return
    ...
    if not missoes:
        await interaction.followup.send("Nenhuma missao disponivel esta semana!", ephemeral=True)
        return
...
async def cmd_guilda_convidar(interaction: discord.Interaction, jogador: discord.Member):
    ...
    if m["cargo"] not in ("mestre", "oficial"):
        await interaction.followup.send("Apenas Mestre e Oficial podem convidar!", ephemeral=True)
        return
    ...
    async with pool.acquire() as conn:
        ...
        if len(membros) >= g["max_membros"]:
            await interaction.followup.send("Guilda cheia!", ephemeral=True)
            return
        ex = await conn.fetchrow("SELECT id FROM guilda_membros WHERE user_id = $1", jogador.id)
        if ex:
            await interaction.followup.send(f"{jogador.display_name} ja faz parte de uma guilda!", ephemeral=True)
            return
    ...
    await v.wait()
    if not v.resposta:
        await interaction.followup.send(f"{jogador.display_name} recusou.", ephemeral=True)
        return
...
async def cmd_guilda_sair(interaction: discord.Interaction):
    ...
    if m["cargo"] == "mestre":
        ...
        if len(membros) > 1:
            await interaction.followup.send("Voce e o Mestre! Transfira a lideranca antes de sair.", ephemeral=True)
            return
...
async def cmd_guilda_expulsar(interaction: discord.Interaction, jogador: discord.Member):
    ...
    if not g or m["cargo"] != "mestre":
        await interaction.followup.send("Apenas o Mestre pode expulsar!", ephemeral=True)
        return
    if jogador.id == interaction.user.id:
        await interaction.followup.send("Nao pode se expulsar!", ephemeral=True)
        return
    ...
    async with pool.acquire() as conn:
        ...
        if not ex:
            await interaction.followup.send(f"{jogador.display_name} nao e membro!", ephemeral=True)
            return
...
async def cmd_guilda_promover(interaction: discord.Interaction, jogador: discord.Member):
    ...
    g, m = await get_guilda_do_jogador(interaction.user.id)
    if not g or m["cargo"] != "mestre":
        await interaction.followup.send("Apenas o Mestre pode promover!", ephemeral=True)
        return
    ...
    async with pool.acquire() as conn:
        ...
        if not alvo:
            await interaction.followup.send(f"{jogador.display_name} nao e membro!", ephemeral=True)
            return
        if alvo["cargo"] == "mestre":
            await interaction.followup.send(f"{jogador.display_name} ja e mestre!", ephemeral=True)
            return
        novo_cargo = "mestre" if alvo["cargo"] == "oficial" else "oficial"
...
async def cmd_guilda_depositar(interaction: discord.Interaction, valor: int):
    ...
    if not g:
        ...
        return
    if valor <= 0:
        await interaction.followup.send("Valor invalido!", ephemeral=True)
        return
    ...
    async with pool.acquire() as conn:
        ...
        if not p or p["moedas"] < valor:
            await interaction.followup.send(f"Moedas insuficientes!", ephemeral=True)
            return
...
async def cmd_guilda_retirar(interaction: discord.Interaction, valor: int):
    ...
    g, m = await get_guilda_do_jogador(interaction.user.id)
    if not g or m["cargo"] != "mestre":
        await interaction.followup.send("Apenas o Mestre pode retirar do banco!", ephemeral=True)
        return
    if valor <= 0:
        await interaction.followup.send("Valor invalido!", ephemeral=True)
        return
    ...
    async with pool.acquire() as conn:
        if g["banco"] < valor:
            await interaction.followup.send(f"Banco insuficiente! Tem {g['banco']} 🪙", ephemeral=True)
            return
        await conn.execute("UPDATE guildas SET banco = banco - $1 WHERE id = $2", valor, g["id"])
        await conn.execute("UPDATE personagens SET moedas = moedas + $1 WHERE user_id = $2", valor, interaction.user.id)
