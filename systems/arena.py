# systems/arena.py — Sistema de Arena Ranqueada
...
from data.constantes import ELOS, DESAFIOS_POR_DIA, ARENAS, COR_PRIMARY, COR_SUCCESS, COR_DANGER, COR_WARNING, COR_INFO, COR_GOLD
from systems.combate import rodar_pvp, BATALHAS_ATIVAS
...
def get_temporada_atual() -> str:
    """Retorna a temporada atual no formato AAAA_MM"""
    agora = datetime.now(timezone.utc)
    return f"{agora.year}_{agora.month}"


def get_proxima_temporada() -> str:
    """Retorna a próxima temporada"""
    agora = datetime.now(timezone.utc)
    if agora.month == 12:
...
async def get_jogador_arena(user_id: int, temporada: str = None) -> dict:
    """Retorna os dados do jogador na arena para a temporada"""
...
async def get_elo_por_rating(rating: int) -> tuple:
    """Retorna o elo baseado no rating"""
...
async def registrar_desafio(user_id: int) -> tuple:
    """Registra um desafio do jogador. Retorna (pode, desafios_feitos)"""
    pool = await get_pool()
    async with pool.acquire() as conn:
...
async def registrar_batalha_arena(user_id: int, oponente_id: int, oponente_nome: str, vitoria: bool, rating_antes: int, rating_depois: int):
    """Registra uma batalha no histórico da arena"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute("""
...
async def get_ranking_arena(limite: int = 10, elo_filtro: str = None) -> List[dict]:
    """Retorna o ranking da arena"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        temporada = get_temporada_atual()
...
async def cmd_arena_desafiar(interaction: discord.Interaction, jogador: discord.Member):
    """Desafia um jogador para batalha ranqueada"""
...
async def cmd_arena_meuperfil(interaction: discord.Interaction):
    """Mostra o perfil do jogador na arena"""
    await interaction.response.defer()
...
async def cmd_arena_recompensas(interaction: discord.Interaction):
    """Resgata recompensas de temporadas anteriores"""
...
async def init_db_arena():
    """Inicializa as tabelas da arena"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute("""
